...
-- Modify the previous query to retrieve the names of the states (rather than their abbreviations).
	SELECT city_name, state.state_abbreviation FROM city
...
-- Modify the previous query to include the area of each park.
	...
	INNER JOIN park_state ps ON p.park_id = p.park_id
	INNER JOIN state s ON ps.state_abbreviation = s.state_abbreviation
...
-- Write a query to retrieve the number of cities in the city table for each state in the Midwest census region.
	SELECT s.state_name, count(*) CityCount FROM state s
	JOIN city c ON s.state_abbreviation = c.state_abbreviation
	WHERE census_region = 'Midwest'
...
-- Write a query to retrieve the state name and the earliest date a park was established in that state (or territory) for every record in the state table that has park records associated with it.
	SELECT state.state_name, min(date_established) EarliestPark FROM state
