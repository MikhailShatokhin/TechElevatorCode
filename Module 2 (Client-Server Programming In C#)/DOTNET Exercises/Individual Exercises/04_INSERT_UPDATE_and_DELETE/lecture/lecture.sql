...
-- Add Disneyland to the park table. (It was established on 7/17/1955, has an area of 2.1 square kilometers and does not offer camping.)
	...
	VALUES ('Disneyland', '7/7/1955', 2.1, 'false')
...
-- Change the state nickname of California to "The Happiest Place on Earth."
	SELECT * FROM state WHERE state_abbreviation = 'CA'; -- use to test below
...
-- Delete Hawkins, IN from the city table.
	SELECT * FROM city WHERE city_name = 'Hawkins' -- use to test below

	DELETE FROM city
	WHERE city_name = 'Hawkins';
-- Delete all cities with a population of less than 1,000 people from the city table.
	SELECT * FROM city WHERE population < 1000 -- use to test below
...
-- THIS DOES NOT WORK BECAUSE OF FORIEGN KEY CONSTRAINT
	INSERT INTO [dbo].[city]
		   ([city_name]
		   ,[state_abbreviation]
		   ,[population]
...
-- if we edit the top 200 and delete the row it will work
	DELETE FROM park WHERE park_name = 'Disneyland'
