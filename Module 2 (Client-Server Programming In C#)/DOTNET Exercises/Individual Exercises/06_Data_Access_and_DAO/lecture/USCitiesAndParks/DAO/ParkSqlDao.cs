...
namespace USCitiesAndParks.DAO
{
    public class ParkSqlDao : IParkDao
    {
        ...
        public IList<Park> GetParksByState(string stateAbbreviation)
        {
            ...
            {
                conn.Open();
                SqlCommand cmd = new SqlCommand("SELECT park.* FROM park JOIN park_state ON park.park_id = park_state.park_id" + 
                    "WHERE park_state.state_abbreviation = @state_abbreviation;", conn);
