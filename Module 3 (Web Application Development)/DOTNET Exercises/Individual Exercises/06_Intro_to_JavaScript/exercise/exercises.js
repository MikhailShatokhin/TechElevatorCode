...
*/
function oddOnly(x){
	return x.filter(items=>items%2==1)
}
/*
...
function frontAgain(x){
	first=x.substring(0,2);
	last=x.substring(x.length-2);
	if(first==last){
		return true;
	}
	else{
...
function cigarParty(x,y){
	return x>=40 && (x <= 60 && y == false || y==true)
}
/*
...
function filterEvens(x){
	return x.filter(items=>items%2==0);
}
/*
...
function filterBigNumbers(x){
	return x.filter(items=>items>=100);
}
/*
...
function filterMultiplesOfX(x,y){
	return x.filter(items=>items%y==0);
}
/*
